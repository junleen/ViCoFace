...
class ToFlow(nn.Module):
    ...
    def forward(self, input, style, feat, skip=None):
        ...
        xs = np_meshgrid_set[input.size(2)]
        xs = xs.repeat(input.size(0), 1, 1, 1).to(out.device)
...
class Direction(nn.Module):
    ...
    def forward(self, input):
        ...
        if input is None:
            ...
            # out = torch.matmul(input, Q.T)

            return out


class Synthesis(nn.Module):
    def __init__(self, size, style_dim, motion_dim, id_dim, blur_kernel=[1, 3, 3, 1], channel_multiplier=1):
        ...
        self.direction = Direction(motion_dim + id_dim)
        print('Motion dim:%d, identity dim:%d' % (motion_dim, id_dim))
    ...
    def decode_latent_codeW(self, alpha):
        ...
        if alpha is not None:
            if len(alpha) == 2:
                weight = torch.cat(alpha, dim=1)    # bs x (motion_dim + id_dim)
                directions = self.direction(weight)
            else:
                weight = torch.cat([alpha[0] - alpha[1] + alpha[2], alpha[3]], dim=1)
                directions = self.direction(weight)
        else:
            directions = 0
    ...
    def forward(self, wa, alpha, feats):
        ...
        # generating moving directions
        latent = wa + self.decode_latent_codeW(alpha) # wa + directions
        ...
        for conv1, conv2, to_rgb, to_flow, feat in zip(self.convs[::2], self.convs[1::2], self.to_rgbs, self.to_flows, feats):
            ...
            # out_dict['flow'].append(skip_flow)
            # out_dict['warp'].append(out_warp)
            # out_dict['skip'].append(skip)
            i += 2

        img = skip
        # out_dict['gen_img'] = img
        return img
