...
def batch_kp_2d_l1_loss(real_2d_kp, predicted_2d_kp, weights=None):
    ...
    kp_pred: N x K x 2
    """
    dif_loss = torch.abs(real_2d_kp - predicted_2d_kp)
    if weights is not None:
        dif_loss = dif_loss * weights
    return dif_loss.sum() / weights.sum()
...
def weighted_landmark_loss(predicted_landmarks, landmarks_gt, weight=1.):
    ...
    real_2d = landmarks_gt
    nweights = weights.to(real_2d.device)

    loss_lmk_2d = batch_kp_2d_l1_loss(real_2d, predicted_landmarks, nweights)
    return loss_lmk_2d * weight
